...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vechicleClass, direction_number, direction, will_turn):
        pygame.sprite.Sprite.__init__(self)
        self.lane = lane
        self.vehicleClass = vechicleClass
        self.speed = speeds[vechicleClass]
        ...
        if direction == "right": # vehicles going towards right side of simulation
            ...
            stops[direction][lane] -= tmp
        elif direction == "left": # vehicles going towards left side of simulation
            if(len(vehicles[direction][lane]) > 1 and vehicles[direction][lane][self.index - 1].crossed == 0):
                self.stop = vehicles[direction][lane][self.index - 1].stop + vehicles[direction][lane][self.index - 1].currentImage.get_rect().width + gap
            else:
                self.stop = defaultStop[direction]
            tmp = vehicles[direction][lane][self.index - 1].currentImage.get_rect().width + gap
            x[direction][lane] += tmp
            stops[direction][lane] += tmp
        elif direction == "up": # vehicles going towards up side of simulation
            if(len(vehicles[direction][lane]) > 1 and vehicles[direction][lane][self.index - 1].crossed == 0):
                self.stop = vehicles[direction][lane][self.index - 1].stop + vehicles[direction][lane][self.index - 1].currentImage.get_rect().width + gap
            else:
                self.stop = defaultStop[direction]
            tmp = vehicles[direction][lane][self.index].currentImage.get_rect().width + gap
            y[direction][lane] += tmp
            stops[direction][lane] += tmp
        elif direction == "down": # vehicles going towards down side of simulation
            if(len(vehicles[direction][lane]) > 1 and vehicles[direction][lane][self.index - 1].crossed == 0):
                self.stop = vehicles[direction][lane][self.index - 1].stop - vehicles[direction][lane][self.index - 1].currentImage.get_rect().width - gap
            else:
                self.stop = defaultStop[direction]
